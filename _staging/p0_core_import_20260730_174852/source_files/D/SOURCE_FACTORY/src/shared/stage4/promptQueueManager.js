...
/* ST4_W33_PROMPT_QUEUE_MANAGER_PATCH_START */
function makePromptDedupeKey(input) {
  ...
  return [
    ...
    toNonEmptyString(source.worker_slot || source.workerSlot, ''),
    toNonEmptyString(source.worker_id || source.workerId, ''),
    toNonEmptyString(source.task_id || source.taskId, ''),
    String(Number.isFinite(Number(source.send_order || source.sendOrder)) ? Number(source.send_order || source.sendOrder) : ''),
    stableHash(typeof source.prompt_text === 'string' ? source.prompt_text : typeof source.promptText === 'string' ? source.promptText : '')
  ].join('::');
}

function isPromptAlreadySent(item) {
  const source = isPlainObject(item) ? item : {};
  return source.already_sent === true ||
    source.alreadySent === true ||
    Boolean(source.sent_at || source.sentAt) ||
    source.status === PROMPT_QUEUE_STATUSES.SENT ||
    source.status === PROMPT_QUEUE_STATUSES.OUTPUT_WAITING ||
    source.status === PROMPT_QUEUE_STATUSES.OUTPUT_RECEIVED ||
    source.status === PROMPT_QUEUE_STATUSES.GATED ||
    source.status === PROMPT_QUEUE_STATUSES.DONE;
}

function preventDuplicatePromptQueueItem(items, candidate) {
  const list = Array.isArray(items) ? items : [];
  const nextItem = isPlainObject(candidate) ? candidate : {};
  const nextDedupeKey = toNonEmptyString(nextItem.dedupe_key || nextItem.dedupeKey, '');

  if (!nextDedupeKey) {
    return { ok: true, duplicate: false, reason: 'dedupe_key_empty' };
  }

  const duplicate = list.find(function findDuplicate(item) {
    return toNonEmptyString(item.dedupe_key || item.dedupeKey, '') === nextDedupeKey && isPromptAlreadySent(item);
  });

  if (!duplicate) {
    return { ok: true, duplicate: false, reason: 'no_sent_duplicate' };
  }

  return {
    ok: false,
    duplicate: true,
    reason: 'already_sent_dedupe_key_exists',
    duplicate_prompt_id: duplicate.prompt_id || duplicate.promptId || '',
    duplicate_dedupe_key: nextDedupeKey
  };
}
/* ST4_W33_PROMPT_QUEUE_MANAGER_PATCH_END */
...
function normalizePromptItem(item, index, queueContext) {
  ...
  const workerSlot = toNonEmptyString(
    source.worker_slot || source.workerSlot || source.slot || source.worker_id || source.workerId,
    ''
  );
...
function promoteNextQueuedPrompt(items) {
  const hasReadyPrompt = items.some((item) => item.status === PROMPT_QUEUE_STATUSES.READY_TO_SEND && !isPromptAlreadySent(item));
  ...
  const nextQueued = items
    ...
    .filter((item) => !isPromptAlreadySent(item))
...
function enqueuePrompt(queue, promptInput) {
  ...
  const duplicateCheck = preventDuplicatePromptQueueItem(items, prompt);
  if (!duplicateCheck.ok) {
    return {
      ok: false,
      queue: nextQueue,
      prompt: null,
      errors: [duplicateCheck.reason + ': ' + duplicateCheck.duplicate_dedupe_key],
      warnings: []
    };
  }
...
function applyStatusSideEffects(item, status, patch, now) {
  if (
    status === PROMPT_QUEUE_STATUSES.SENT ||
    status === PROMPT_QUEUE_STATUSES.OUTPUT_WAITING ||
    status === PROMPT_QUEUE_STATUSES.OUTPUT_RECEIVED ||
    status === PROMPT_QUEUE_STATUSES.GATED ||
    status === PROMPT_QUEUE_STATUSES.DONE
  ) {
...
module.exports = {
  ...
  makePromptDedupeKey,
  isPromptAlreadySent,
  preventDuplicatePromptQueueItem
};
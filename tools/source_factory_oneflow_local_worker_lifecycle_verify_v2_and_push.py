#!/usr/bin/env python3
"""Source Factory 024B one-flow local worker lifecycle verifier and pusher.

V2 fixes JSON files with UTF-8 BOM by reading all JSON with utf-8-sig.
It validates the local-only worker lifecycle:
queue intake -> claim -> worker dry-run -> terminal receipt save -> duplicate rejection.
No GPT, browser, PC Agent, external API, middleware, remote queue mutation, or production deployment is performed.
"""
...
TASK_ID = "024B_ONEFLOW_LOCAL_WORKER_LIFECYCLE_VERIFY_V2"
...
def main() -> int:
    ...
    print(f"[024B-PY] Repository root: {root}")
    print("[024B-PY] Git pull")
    run_git(root, ["pull"])

    timestamp = now_compact()
    report_dir = root / "reports" / f"oneflow_local_worker_lifecycle_verify_v2_{timestamp}"
    ...
    if not missing_required_files:
        ...
        if all(status == "PASS_PY_COMPILE" for status in compile_statuses.values()):
            claim_module = import_module_from_file("source_factory_local_claim_store_v2", claim_module_path)
            receipt_module = import_module_from_file("source_factory_terminal_receipt_store_v2", receipt_module_path)
            lifecycle_module = import_module_from_file("source_factory_local_worker_lifecycle_v2", lifecycle_module_path)
            ...
            write_json(report_dir / "ONEFLOW_LOCAL_WORKER_LIFECYCLE_SIMULATION_V2.json", lifecycle_result)
    ...
    ) else "FAIL_ONEFLOW_LOCAL_WORKER_LIFECYCLE_VERIFY_V2"

    decision = {
        "worker_id": "SOURCE_FACTORY_024B_ONEFLOW_LOCAL_WORKER_LIFECYCLE_VERIFY_V2_WORKER",
        "task_id": TASK_ID,
        "status": status,
    ...
    write_json(report_dir / "ONEFLOW_LOCAL_WORKER_LIFECYCLE_VERIFY_DECISION_V2.json", decision)
    write_csv(report_dir / "ONEFLOW_LOCAL_WORKER_LIFECYCLE_FILE_LEDGER_V2.csv", file_rows)

    summary = f"""# Source Factory One-flow Local Worker Lifecycle Verify V2
...
"""
    (report_dir / "ONEFLOW_LOCAL_WORKER_LIFECYCLE_VERIFY_SUMMARY_V2.md").write_text(summary, encoding="utf-8")
    (report_dir / "WORKER_REPORT_024B.md").write_text(summary, encoding="utf-8")

    print("SOURCE_FACTORY_ONEFLOW_LOCAL_WORKER_LIFECYCLE_VERIFY_V2_COMPLETE")
    ...
    if not args.no_push:
        ...
        if porcelain:
            print("[024B-PY] Git commit")
            run_git(root, ["commit", "-m", "add oneflow local worker lifecycle verify V2 result"])
            print("[024B-PY] Git push")
            run_git(root, ["push"])
        else:
            print("[024B-PY] No report changes to commit")

    print("SOURCE_FACTORY_ONEFLOW_LOCAL_WORKER_LIFECYCLE_VERIFY_V2_AND_PUSH_COMPLETE")
    print("Status=PASS_LOCAL_WORKER_LIFECYCLE_VERIFY_V2_AND_PUSH_DONE")
    return 0

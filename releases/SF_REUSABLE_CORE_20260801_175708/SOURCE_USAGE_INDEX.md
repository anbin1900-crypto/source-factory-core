...
| 문서 | 용도 |
|---|---|
| `USER_START_HERE.md` | 처음 사용자용 시작 문서 |
| `CORE_ASSET_ANNOTATION_GUIDE.md` | 핵심 자산별 상세 주석과 재사용 방법 |
| `_USAGE_NOTES/PC_AGENT_BINDING_ADAPTER.usage.md` | PC Agent 결속 지점 상세 주석 |
| `_USAGE_NOTES/PROJECT_PANEL_IDENTITY.usage.md` | Project Panel Identity 모델 상세 주석 |
| `ANNOTATION_CHANGELOG.md` | 주석/문서 보강 이력 |
...
8. PC Agent는 기존 dispatch/run check 경로에 adapter로 붙인다.
9. 기존 fallback response를 삭제하지 않는다.
10. Worker/Commander count를 6/7로 고정하지 않는다.

...
class CRepeatNamespaceAdapter{
  ...
  acceptRepeatResult(p){if(!p||p.schema_version!=='W2_REPEAT_RESULT_V1')return{accepted:false,reason:'INVALID_REPEAT_RESULT_SCHEMA'};const a=this.state.repeat_active[p.dispatch_id];if(!a)return{accepted:false,reason:'UNKNOWN_REPEAT_DISPATCH'};if(a.role!==p.role||a.command_id!==p.command_id)return{accepted:false,reason:'REPEAT_CORRELATION_MISMATCH'};const rr=this.repeatRuntime.complete(p);if(!rr.accepted)return rr;delete this.state.repeat_active[p.dispatch_id];this.state.repeat_results[p.dispatch_id]=clone(p);this.state.events.push({at_ms:this.now(),type:'REPEAT_RESULT_ACCEPTED',dispatch_id:p.dispatch_id});this._persist();return{accepted:true};}

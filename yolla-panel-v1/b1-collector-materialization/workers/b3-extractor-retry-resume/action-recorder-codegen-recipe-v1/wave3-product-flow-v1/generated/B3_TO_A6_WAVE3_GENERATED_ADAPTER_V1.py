    ...
    try:
        for step in RECIPE["steps"]:
            ...
            if kind in {"popup", "iframe", "navigation"}:
                if kind == "navigation" and page.url != step["url"]: raise AssertionError(("navigation", page.url, step["url"]))
                if kind == "popup" and "popup-1" not in pages: raise AssertionError("popup not opened")
                if kind == "iframe" and pages.get(step["page_alias"], page).frame(name=step["frame_name"]) is None: raise AssertionError("iframe not present")

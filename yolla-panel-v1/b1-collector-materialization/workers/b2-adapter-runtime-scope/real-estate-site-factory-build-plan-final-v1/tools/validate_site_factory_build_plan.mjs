...
eq(plan.ten_site_model.target_site_count,10);eq(plan.ten_site_model.known_candidate_count,2);eq(plan.ten_site_model.waiting_site_slots.length,8);eq(plan.ten_site_model.actual_live_confirmed_site_count,0);eq(plan.ten_site_model.site_name_guessing,false);
...
const s=summarizeSiteMatrix(coverage.site_matrix);eq(s.total,80);eq(s.confirmed,0);eq(s.candidate,6);eq(s.unknown_or_waiting,74);eq(coverage.summary.generic_screen_requirement_defined_count,8);eq(coverage.backlog_planning_coverage.priority_bound_percent,100);eq(coverage.backlog_planning_coverage.dependency_declared_percent,100);eq(coverage.backlog_planning_coverage.plannable_percent,68.75);
eq(coverage.last_final_batch_inputs.A4,'AVAILABLE_FINAL_SOURCE');eq(coverage.last_final_batch_inputs.A5,'WAITING_INPUT');eq(coverage.last_final_batch_inputs.B5,'WAITING_INPUT');
eq(slice.slice_id,'V2-MVS-PUBLIC-READ-SEARCH-LIST-DETAIL-001');eq(slice.target_site_selection.current_target,'WAITING_INPUT');eq(slice.target_site_selection.candidate_is_execution_authority,false);eq(slice.final_write_or_edit_submit,false);eq(slice.execution_order.map(x=>x.backlog_id),['BL-001','BL-002','BL-003','BL-004','BL-005','BL-006','BL-007','BL-008','BL-014']);

...
const untouched1=JSON.stringify(cov.site_matrix.find(s=>s.site_id==='SITE_SLOT_003').screens.LIST);const untouched2=JSON.stringify(cov.site_matrix.find(s=>s.site_id==='SITE_SLOT_004').screens.SEARCH);
...
assert.equal(JSON.stringify(r.siteMatrix.find(s=>s.site_id==='SITE_SLOT_003').screens.LIST),untouched1);assert.equal(JSON.stringify(r.siteMatrix.find(s=>s.site_id==='SITE_SLOT_004').screens.SEARCH),untouched2);assert.equal(r.current.decision,'EVIDENCE_CANDIDATE');assert.notEqual(r.current.decision,'CONFIRMED');
console.log(JSON.stringify({status:'PASS',before,after:r.summary,touched:'SITE_SLOT_003/SEARCH',unrelated_cells_unchanged:true,no_auto_confirm:true}));

...
(function run() {
  ...
  for (const contract of ["window.YollaPanel", "registerProvider", "yolla:role-selected", "yolla:worker-window-bound", "yolla:command-requested", "EXISTING_SF_API_STAGE4"]) {
  ...
  for (const marker of ["YOLLA_PANEL_MAIN_BRIDGE_REQUIRE_V1", "YOLLA_PANEL_MAIN_BRIDGE_REGISTER_V1", "YOLLA_PANEL_PRELOAD_BRIDGE_V1", "YOLLA_PANEL_SHELL_V1", "YOLLA_PANEL_RENDERER_SCRIPT_V1"]) {

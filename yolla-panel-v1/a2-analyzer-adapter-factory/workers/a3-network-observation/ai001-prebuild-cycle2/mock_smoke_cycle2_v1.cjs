...
const { runObservationRequest, filterPageTargets } = require('./SUCCESSOR_CDP_OBSERVATION_RUNNER_V1.cjs');
...
class MockTransport {
  ...
  async send(method) { if (method === 'Target.getTargets') return { targetInfos:this.targets }; return {}; }
  async close() {}
}
...
class MockObserver extends EventEmitter {
  ...
  async start({target}) {
    assert.equal(target.target_id,'page-1'); this.started=true;
...
async function runSmoke(){
  ...
  const request={schema_version:'CDP_OBSERVATION_RUN_REQUEST_V1',cdp_endpoint:process.env.A3_MOCK_CDP_ENDPOINT||'MOCK_RUNTIME_ARGUMENT',page_selector:{title_contains:'Fixture App'},observation_window_ms:0,capture_flags:{dom_snapshot:true,network_metadata:true,response_body:true,frame_navigation:true,console:false}};
  let tick=Date.parse('2026-08-07T13:00:00.000Z');
  const options={skip_default_dependencies:true,output_root:root,transportFactory:async()=>new MockTransport(),observerFactory:()=>new MockObserver(),redactUrl:(url)=>url.replace(/([?&]token=)[^&#]*/ig,'$1<REDACTED>'),clock:()=>tick++,sleep:async()=>{}};
  assert.deepEqual(filterPageTargets(new MockTransport().targets).map((t)=>t.targetId),['page-1']);
  const first=await runObservationRequest(request,options); const second=await runObservationRequest(request,options);
  assert.equal(first.status,'PASS'); assert.equal(first.page_identity.page_id,'page-1'); assert.equal(first.retry_count,0); assert.equal(first.retry_policy,'NO_INTERNAL_RETRY');
  assert.equal(second.idempotent_replay,true); assert.equal(second.run_id,first.run_id); assert.equal(second.request_sha256,first.request_sha256);
  const receiptText=fs.readFileSync(path.join(root,first.run_id,'receipt.json'),'utf8'); assert(!receiptText.includes(request.cdp_endpoint));
  const all=fs.readdirSync(path.join(root,first.run_id)).filter((n)=>n.endsWith('.json')||n.endsWith('.ndjson')).map((n)=>fs.readFileSync(path.join(root,first.run_id,n),'utf8')).join('\n');
  assert(!all.includes('raw-dom-secret'));
  const result={status:'PASS',smoke_run_count:1,eligible_target_filter_pass:true,browser_ui_background_exclusion_pass:true,request_contract_pass:true,receipt_contract_pass:true,output_pointer_pass:true,raw_endpoint_in_receipt_count:0,raw_secret_value_count:0,internal_retry_count:0,idempotent_replay_check:true,terminal:'A3_CDP_OBSERVATION_RUNNER_AND_RECEIPT_READY'};

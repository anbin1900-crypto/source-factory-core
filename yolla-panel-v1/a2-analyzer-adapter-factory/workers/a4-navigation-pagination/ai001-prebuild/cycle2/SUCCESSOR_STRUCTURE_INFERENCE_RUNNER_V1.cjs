...
function paginationCandidates(primary){
  ...
  return [{
    ...
    evidence:p.evidence??p.href??p.nodeId??p.nodeIds??null,
    stop_condition:stopByType[type]||'NO_PROGRESS_OR_REPEAT_GUARD'
  }];
}
function runRequest(req,requestPath='<memory>'){
  ...
  const fields=(raw.fieldCandidates||[]).map(f=>({field_id:f.fieldId,name:f.name,value_type:f.valueType,source_node_id:f.sourceNodeId,source_locator:f.sourceLocator||f.relativeCss||null,sample_values:f.sampleValues||[],coverage:Number(f.coverage??0),confidence:Number(f.confidence??0)}));

...
class PipelineThread(QThread):
    ...
    def run(self) -> None:
        try:
            ...
            if self.mode == "FOLDER":
                kwargs["source_folder"] = self.source
            else:
                kwargs["pdf_file"] = self.source
            self.completed.emit(run_pipeline(**kwargs))
...
class MainWindow(QMainWindow):
    ...
    def start(self) -> None:
        ...
        if not source or not output:
            ...
            return
        mode = "FOLDER" if self.folder_mode.isChecked() else "SINGLE_PDF"
